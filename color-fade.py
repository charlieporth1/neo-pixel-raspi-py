...
def default_cycle(colors = [], wait_ms = 50):
          for color in colors:
              print("On Color " + str(color))
              r, g, b = color
              colorWipe(strip, Color(r,g,b), wait_ms)

def slow_cycle(args, colors = [], wait_ms = 100):
           minval, maxval = 1,2
           multipler = 1
           ...
           for i in range(steps+1):
               ...
               rainbowCycle(strip, wait_ms, iterations=99*99*99)
    ...
    try:
        while True:
                if args.slow or args.win95:
                   slow_cycle(args, colors, wait_ms * 2)
                else:
                   default_cycle(colors, wait_ms)
